...
在自定义挂钩中使用。 请记住，它仅适用于开发模式
（例如 `NODE_ENV === 'development'`）。

### 使用原子

`useAtom` 为原子值调用 `useDebugValue`，所以如果您在 React Dev Tools 中选择消耗 Jotai 原子的组件，您会看到组件中使用的每个原子的“Atom”挂钩以及它现在具有的值 .

### 使用 AtomsDebugValue

`useAtomsDebugValue` 捕获 Provider 下的组件树中的所有原子（或无提供者模式下的整个树），并且 `useDebugValue` 用于所有原子值。
如果您在 React Dev Tools 中导航到具有 useAtomsDebugValue 的组件，我们可以看到一个自定义挂钩 `AtomsDebugValue`，它允许您查看所有原子值及其依赖项。
